...
def AdmixL1i(params, ns, pts):
    ...
    fs = dadi.Spectrum.from_phi(phi, ns, (xx, xx, xx), pop_ids=['1c', '2', '1i'])
    return fs
...
for jj in range(0,10):
    ...
    data = dadi.Spectrum.from_data_dict(dd, ['1c', '2', '1i'], [20, 20, 20], polarized=True)
    ...
    fout = open('AdmixL1i.xls','a')
    fout.write('1st_anneal_optimization\tAdmixL1i\t' + str(ll_opt) + '\t' + str(theta) + '\t' + str(popt).replace('\n','') + '\t' + str(AIC) + '\n')
...
for jj in range(0, 5):
    ...
    fout = open('AdmixL1i.xls','a')
    fout.write('2nd_otimization\tAdmixL1i\t' + str(ll_opt) + '\t' + str(theta) + '\t' + str(popt).replace('\n','') + '\t' + str(AIC) + '\n')
    fout.close()